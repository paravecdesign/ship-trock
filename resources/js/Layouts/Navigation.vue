  ...
  <aside class="z-20 flex-shrink-0 hidden w-64 overflow-y-auto bg-white md:block">
    <div class="py-4 text-gray-500">
      ...
      <ul class="mt-6">
        ...
        <li class="relative px-6 py-3">
          <BreezeNavLink :href="route('admin.customers.index')" :active="route().current('admin.customers')">
            <template #icon>
        ...
        <li class="relative px-6 py-3">
          <BreezeNavLink :href="route('admin.shipments.index')" :active="route().current('shipments')">
            <template #icon>
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                   xmlns="http://www.w3.org/2000/svg">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          ...
          <button @click="showingTwoLevelMenu = !showingTwoLevelMenu"
            ...
            <ul v-show="showingTwoLevelMenu" class="p-2 mt-2 space-y-2 overflow-hidden text-sm font-medium text-gray-500 rounded-md shadow-inner bg-gray-50 dark:text-gray-400 dark:bg-gray-900"
              ...
              <li class="px-2 py-1 transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200">
                     <BreezeNavLink :href="route('admin.setting.statuses.index')" :active="route().current('admin.status')">
            <template #icon>
              <svg class="w-5 h-5" aria-hidden="true" fill="none" stroke-linecap="round"
                   stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
                <path
