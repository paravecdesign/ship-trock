            ...
            <div class="inline-flex w-full mb-4 overflow-hidden bg-white rounded-lg shadow-md">
                ...
                <div class="flex justify-between mx-3 px-4 py-2 w-full">
                    ...
                    <div class="flex justify-between">
                        <SelectStatusAction :statuses="statuses" @execute="executeAction" />
                        ...
                        <tbody class="bg-white divide-y">
                            <tr v-for="shipment in shipments.data" :key="shipment.id" class="text-gray-700">
                                ...
                                <td class="px-3 py-2 flex">
                                    ...
                                    <Link @click="destroy(shipment.id)"
...
import SelectStatusAction from "@/Components/SelectStatusAction.vue";
export default {
    components: {
        BreezeAuthenticatedLayout,
        Pagination,
    ...
    setup() {
        ...
        function submit() {
            form.post(route("admin.excel.import"));
        }
        const destroy = (id) => {
            if (confirm("Are you sure?")) {
                Inertia.delete(route("admin.shipments.destroy", id));
            }
        };
        return {
            destroy,
    ...
    methods: {
        executeAction(statusId) {
            const shipmentIds = this.shipments.data.filter(shipment => shipment.selected).map(shipment => shipment.id);
            // console.log(statusId, shipmentIds);
        if(!shipmentIds.length > 0) return;


                this.$inertia.post(route("admin.shipments.bulkupdate"),{
