    ...
    <div class="vd jd tto vv oq ar ri">
      ...
      >
        ...
        <div class="space-y-8 pt-10 col-span-5">
          <div class="justify-end flex pr-5">
            <Link
              :href="route('admin.setting.rates.index')"
              class="
                text-md
                px-6
                py-2
          ...
          <div class="grid grid-cols-3">
            <div class="col-span-1">
              <div class="rounded-md border mr-5">
                <h3 class="text-gray-600 p-3">Rates</h3>
                <div class="border">
                  <form @submit.prevent="submit">
                    <div class="mt-4 p-3">
                      <BreezeLabel for="cost" value="Rate Cost" />
                      <input
                    ...
                    <div class="mt-4 p-3">
                      <BreezeLabel for="weight" value="Weight Of Rate" />
                      <input
                        class="block w-full mt-1 text-xs"
                        type="text"
                        v-model="form.weight"
                ...
                <table class="w-full whitespace-no-wrap">
                  ...
                  <tbody class="bg-white divide-y">
                    <tr
                      ...
                      class="text-gray-700"
                    >
                      <td class="px-3 py-2">{{ rate.id }}</td>
                      <td class="px-3 py-2">{{ rate.cost }}</td>
                      <td class="px-3 py-2">{{ rate.weight }}</td>
...
export default {
  ...
  methods: {
    edit(id) {
      this.$inertia.get(route("admin.setting.rates.edit", id), {
        onSuccess: () => this.form.reset(),
      });
    },
    submit() {
      this.form.put(route("admin.setting.rates.update", this.rate.id), {
        onSuccess: () => this.form.reset(),
      });
    },
    destroy(id) {
      this.$inertia.delete(route("admin.setting.rates.destroy", id));
    },
  },
};
</script>
